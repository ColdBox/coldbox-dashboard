...
<div class="maincontentbox">
	...
	<select name="show" id="show" onChange="doEvent('#requestContext.getValue("xehCFCDocs")#', 'content', {show:this.value})">
	 <option value="core" <cfif requestContext.getValue("show") eq "core">selected</cfif>>Core</option>
	 <option value="plugins" <cfif requestContext.getValue("show") eq "plugins">selected</cfif>>Plugins</option>	 
	 <option value="beans" <cfif requestContext.getValue("show") eq "beans">selected</cfif>>Beans</option>	 
	 <option value="util" <cfif requestContext.getValue("show") eq "util">selected</cfif>>Util</option>	 
	</select>
	</div>
	#requestContext.getValue("cfcViewer").render()#	
	</div>
